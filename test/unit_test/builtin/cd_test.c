...
int	test_cd(char **envp)
{
	...
	args = make_args((char *[]){"/tmp", NULL});
	CHECK(cd_cmd(args, &ctx) == ST_OK
		&& hash_search("OLDPWD", ctx.env_table) != NULL
		&& ft_strcmp(hash_search("OLDPWD", ctx.env_table)->data.value,
			origin) == 0, "cd: OLDPWD updated");
	free_args(args);
	chdir(origin);
	teardown_ctx(&ctx);
