...
{
	return (buf_append_mem(buf, str, ft_strlen(str)));
}
...
static int	buf_init(t_buffer *buf)
{
	buf->cap = 256;
	buf->len = 0;
	buf->data = ft_calloc(buf->cap, sizeof(char));
	if (!buf->data)
		return (0);
	return (1);
}
...
static char	*make_child_prefix(const char *prefix, int is_last)
{
	...
	if (is_last)
		...
		pad = "│  ";
	len = ft_strlen(prefix) + ft_strlen(pad);
	next_prefix = ft_calloc(len + 1, sizeof(char));
	if (!next_prefix)
		return (NULL);
	ft_memcpy(next_prefix, prefix, ft_strlen(prefix));
	ft_memcpy(next_prefix + ft_strlen(prefix), pad, ft_strlen(pad));
	return (next_prefix);
}
